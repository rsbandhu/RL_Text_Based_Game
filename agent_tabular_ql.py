...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if np.random.rand() < epsilon: # choose a random number with prob = epsilon
        ...
        (action_index, object_index) = np.unravel_index(np.random.randint(low = 0, high= NUM_ACTIONS*NUM_OBJECTS), (NUM_ACTIONS, NUM_OBJECTS))
    else:
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    #Find maximum of Q function for the next state over all actions from the next state
    if terminal:
        max_Q_overAction = 0
    else:
        max_Q_overAction = np.max(q_func[next_state_1, next_state_2,:,:])

    #Update the Q-value function, exponential averaging with factor ALPHA
    q_func[current_state_1, current_state_2, action_index,
           ...
           object_index] + ALPHA*(reward + GAMMA* max_Q_overAction) # TODO Your update here
...
def run_episode(for_training):
    ...
    cur_state_1 = dict_room_desc[current_room_desc]
    cur_state_2 = dict_quest_desc[current_quest_desc]
    ...
    while not terminal:
        ...
        (action_index, object_index) = epsilon_greedy(cur_state_1, cur_state_2, q_func, epsilon)
        ...
        if for_training:
            # update Q-function.
            # TODO Your code here
            tabular_q_learning(q_func, cur_state_1, cur_state_2, action_index,
